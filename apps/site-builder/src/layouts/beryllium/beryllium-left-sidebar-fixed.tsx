...
import { isAdminSync } from '@/lib/utils/auth';
...
function MenuItems() {
  ...
  const filteredMenuItems = berylliumMenuItems.filter((menu) => {
    // Show admin menu only to admins
    if (menu.id === '2' && !isAdmin) {
      return false;
    }
    return true;
  });
  ...
  return (
    <aside className="fixed start-0 top-0 z-50 hidden h-screen w-[88px] flex-col items-center gap-10 bg-gray-900 py-3.5 dark:bg-gray-0 xl:flex">
      <Link
        ...
        href={'/'}
        className="me-4 hidden w-[155px] shrink-0 text-gray-800 hover:text-gray-900 xl:block"
      >
