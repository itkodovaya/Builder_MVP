	...
	actions: {
		...
		deletePage: async (page: BuilderPage) => {
			const confirmed = await confirm(
				`Вы уверены, что хотите удалить страницу: ${page.page_title || page.page_name}?`,
			);
			if (confirmed) {
				await toast.promise(webPages.delete.submit(page.name), {
					loading: "Удаление страницы",
					success: () => {
						return "Страница удалена";
					},
					error: () => {
						return "Не удалось удалить страницу";
					},
				});
			}
		},
		...
		async revertChanges() {
			const confirmed = await confirm(
				"Это отменит все изменения, внесенные на страницу с момента последней публикации. Вы уверены, что хотите продолжить?",
			);
			if (confirmed) {
		...
		async unpublishPage() {
			const confirmed = await confirm(
				"Вы уверены, что хотите снять эту страницу с публикации? Она больше не будет доступна на сайте.",
			);
			if (!confirmed) {
				return;
			}
		...
		setPageData(page?: BuilderPage) {
			...
			return webPages.runDocMethod
				...
				.catch((e: { exc: string | null }) => {
					...
					toast.error("Произошла ошибка при получении данных страницы", {
