		...
		<section class="m-auto mb-32 flex h-fit w-3/4 max-w-6xl flex-col pt-5">
			...
			<div>
				<div v-if="!webPages.data?.length && !searchFilter && !typeFilter" class="col-span-full">
					<p class="px-3 text-base text-gray-500">
						У вас пока нет страниц. Нажмите на кнопку "+ Создать" для создания новой страницы.
					</p>
				</div>
				<div v-else-if="!webPages.data?.length" class="col-span-full">
					<p class="text-base text-gray-500">Страницы не найдены.</p>
				</div>
				<!-- grid -->
			...
			<div class="sticky top-0 order-[-1] mb-8 flex items-center justify-between bg-surface-white px-3 py-5">
				<h1 class="text-xl font-semibold text-ink-gray-9">
					{{ builderStore.activeFolder || "Все страницы" }}
				</h1>
				<div class="flex gap-2">
					...
					<div class="relative flex" v-show="!selectionMode">
						<BuilderInput
							class="w-48"
							type="text"
							placeholder="Фильтр по заголовку или маршруту"
					...
					<div class="max-md:hidden" v-show="!selectionMode">
						<Select
							...
							:options="[
								...
								{ label: 'Не опубликовано', value: 'unpublished' },
							]" />
					</div>
					<div class="max-sm:hidden" v-show="!selectionMode">
						<Select
							...
							class="!w-32"
							:options="[
								{ label: 'Сортировка', value: '', disabled: true },
								{ label: 'Последние созданные', value: 'creation' },
								{ label: 'Последние измененные', value: 'modified' },
								{
