...
async function confirm(message: string, title: string = "Подтверждение"): Promise<boolean> {
...
async function alert(message: string, title: string = "Предупреждение"): Promise<boolean> {
	return new Promise((resolve) => {
		h(AlertDialog, {
			title,
			message,
...
async function uploadUserFont(
	...
	if (existingFont) {
		toast.info(`Шрифт "${fontName}" уже существует в проекте`);
		return { uploaded: false, fontName, alreadyExists: true };
	}

	// Confirm before uploading if requested
	if (options.confirmBeforeUpload) {
		const confirmed = await confirm(`Вы хотите загрузить шрифт "${fontName}"?`, "Загрузка шрифта");
		if (!confirmed) {
			return null;
		}
	}
	...
	toast.promise(uploadPromise, {
		...
		success: `Шрифт "${fontName}" успешно загружен`,
		error: "Не удалось загрузить шрифт",
	});
